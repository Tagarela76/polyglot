...
add_action('wp_ajax_addGroupAjax', 'addGroupAjax');
add_action('wp_ajax_nopriv_addGroupAjax', 'addGroupAjax');
...
function addGroupAjax()
{
    ...
    $allGroups = $wpdb->get_results("SELECT * FROM ".$table);
    //create html for display in group list
    $html = "";
    foreach($allGroups as $group){
        $html.="<option value='".$group->id."'>".$group->description."</option>";
    }
    echo $html;
    die();
}

add_action('wp_ajax_addSubGroupAjax', 'addSubGroupAjax');
add_action('wp_ajax_nopriv_addSubGroupAjax', 'addSubGroupAjax');
function addSubGroupAjax()
{
    global $wpdb;
    $table = "polyglot_users_subgroup";
...
add_action('wp_ajax_getSubGroupListHtml', 'getSubGroupListHtml');
add_action('wp_ajax_nopriv_getSubGroupListHtml', 'getSubGroupListHtml');
function getSubGroupListHtml()
{
    global $wpdb;
    $table = "polyglot_users_subgroup";
    
    $groupId = $_POST['groupId'];
    $allSubGroups = $wpdb->get_results("SELECT * FROM ".$table." WHERE group_id=".$groupId);
    //create html for display in group list
    $html = "";
    foreach($allSubGroups as $group){
        $html.="<option value='".$group->id."'>".$group->description."</option>";
    }
    echo $html;
    die();
